...
async function seedData() {
  ...
  for (let i = 0; i < createdStories.length; i++) {
    ...
    switch (story.title) {
      case "荆轲刺秦王":
        segments = jingkeSegments.map((s: any) => ({ ...s, storyId: story.id }));
        break;
      case "赤壁之战":
        segments = chibiSegments.map((s: any) => ({ ...s, storyId: story.id }));
        break;
      case "玄武门之变":
        segments = xuanwumenSegments.map((s: any) => ({ ...s, storyId: story.id }));
        break;
    }
...
module.exports = { seedData, sampleStories, jingkeSegments, chibiSegments, xuanwumenSegments };